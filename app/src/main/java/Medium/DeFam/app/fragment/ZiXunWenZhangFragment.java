...
public class ZiXunWenZhangFragment extends BaseFragment {
    public static ZiXunWenZhangFragment newInstance(int type) {
        ZiXunWenZhangFragment fragment = new ZiXunWenZhangFragment();
    ...
    public int getLayoutId() {
        return R.layout.fragment_zixunwenzhang;
    }
    ...
    private void getCategory() {
        ...
        HttpClient.getInstance().gets(HttpUtil.ARTICLECATEGORY, null, new TradeHttpCallback<JsonBean<CategoryBean>>() {
            @Override
            public void onSuccess(JsonBean<CategoryBean> data) {
                for (int i = 0, a = data.getData().getData().size(); i < a; i++) {
                    ...
                    fragmentList.add(ZiXunItemFragment.newInstance(data.getData().getData().get(i).getName(), data.getData().getData().get(i).getId(), i == 0 ? true : false));
                }
    ...
    private class MyOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            if (fragmentList.get(position) instanceof ZiXunItemFragment) {
                ZiXunItemFragment fragment = (ZiXunItemFragment) fragmentList.get(position);//懒加载
                if (null != fragment) {
                    fragment.loadData();
                }
            }
