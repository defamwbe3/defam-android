...
import Medium.DeFam.app.fragment.FaTieShouCangFragment;
import Medium.DeFam.app.fragment.HuoDongShouCangFragment;
import Medium.DeFam.app.fragment.ZiXunShouCangFragment;
...
public class ShouCang extends BaseActivity {
    ...
    private String[] toptitle = {"资讯", "发帖", "活动"};
    ...
    protected void initView(@Nullable Bundle savedInstanceState, @Nullable Bundle bundle) {
        CommonNavigator commonNavigatorTop = new CommonNavigator(ShouCang.this);
        commonNavigatorTop.setAdapter(new CommonNavigatorAdapter() {
            ...
            public IPagerTitleView getTitleView(Context context, final int index) {
                ...
                colorTransitionPagerTitleView.setPadding(AutoSizeUtils.dp2px(ShouCang.this, 20), 0, AutoSizeUtils.dp2px(ShouCang.this, 20), 0);
    ...
    protected void initData() {
        fragmentList.add(ZiXunShouCangFragment.newInstance(true));
        fragmentList.add(FaTieShouCangFragment.newInstance(false));
        fragmentList.add(HuoDongShouCangFragment.newInstance(false));
    ...
    private class MyOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            if (fragmentList.get(position) instanceof ZiXunShouCangFragment) {
                ZiXunShouCangFragment fragment = (ZiXunShouCangFragment) fragmentList.get(position);//懒加载
                if (null != fragment) {
                    fragment.loadData();
                }
            }else if (fragmentList.get(position) instanceof FaTieShouCangFragment) {
                FaTieShouCangFragment fragment = (FaTieShouCangFragment) fragmentList.get(position);//懒加载
                if (null != fragment) {
                    fragment.loadData();
                }
            }else if (fragmentList.get(position) instanceof HuoDongShouCangFragment) {
                HuoDongShouCangFragment fragment = (HuoDongShouCangFragment) fragmentList.get(position);//懒加载
                if (null != fragment) {
                    fragment.loadData();
                }
            }
