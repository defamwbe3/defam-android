...
public class KOLDetail extends BaseActivity {
    @BindView(R.id.title)
    MyTextView title;
    ...
    protected int getLayoutId() {
        return R.layout.activity_koldetail;
    }
    ...
    protected void initData() {
        alldata = (KOLBean.DataBean) getIntent().getSerializableExtra("data");
        if(null==alldata){
            finish();
            return;
        }
    ...
    private void setUI(){
        title.setText(alldata.getName());
        created_at.setText(alldata.getCreated_at());
        web.loadDataWithBaseURL(Constants.HOST, HTML_STYLE + alldata.getFull_text(), "text/html",
                "utf-8", null);
    }
