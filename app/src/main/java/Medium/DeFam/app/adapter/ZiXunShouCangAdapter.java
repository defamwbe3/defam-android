...
public class ZiXunShouCangAdapter extends RecyclerView.Adapter {
    private final List<ShouCangZiXunBean.DataBean> dataList = new ArrayList<>();
    ...
    public ZiXunShouCangAdapter(Context context) {
        this.context = context;
    }

    public void replaceAll(List<ShouCangZiXunBean.DataBean> list) {
        dataList.clear();
     ...
     * 不然容易出现闪动和间距错乱的问题
     */
    public void addData(List<ShouCangZiXunBean.DataBean> list) {
        dataList.addAll(list);
        notifyDataSetChanged();
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ZiXunDetailBean data = dataList.get(position).getAction_txt();
    ...
    public int getItemViewType(int position) {
        if (null == dataList.get(position).getAction_txt().getImage_text() || dataList.get(position).getAction_txt().getImage_text().size() <= 1) {
            return TYPE_ITEM;
        } else {
            return TYPE_ITEM1;
        }
